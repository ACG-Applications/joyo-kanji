...
// ==================== SVG LOADING ====================
function loadSvgWithRetry(kanji, unicode, container) {
    const padded5 = unicode.padStart(5, '0');
    const padded4 = unicode.replace(/^0+/, '');
    const paths = [
        `images/kanji-strokes/${padded5}.svg`,
        `images/kanji-strokes/${padded4}.svg`
    ];
    let tried = 0;
    ...
    function tryNext() {
        if (tried >= paths.length) {
            container.innerHTML = `<span class="fallback-char">${kanji}</span>`;
            return;
        }
...
function getSvgPathForPrint(unicode) {
    const padded5 = unicode.padStart(5, '0');
    return `images/kanji-strokes/${padded5}.svg`;
}

function getSvgPathWithFallback(unicode) {
    const padded5 = unicode.padStart(5, '0');
    const padded4 = unicode.replace(/^0+/, '');
    return { primary: `images/kanji-strokes/${padded5}.svg`, fallback: `images/kanji-strokes/${padded4}.svg` };
}
...
function renderGrid() {
    ...
    filtered.forEach(d => {
        ...
        // Get the unicode and pad it properly
        const unicode = d.unicode;
        const padded5 = unicode.padStart(5, '0');
        const padded4 = unicode.replace(/^0+/, '');
        const primaryPath = `images/kanji-strokes/${padded5}.svg`;
        const fallbackPath = `images/kanji-strokes/${padded4}.svg`;
        const fallbackChar = d.kanji;
        ...
        html += `
            <div class="kanji-card ${isSelected ? 'selected' : ''}" data-kanji="${d.kanji}">
                ...
                <div class="stroke-svg" data-unicode="${unicode}">
                    <img src="${primaryPath}" alt="Stroke order for ${d.kanji}" loading="lazy"
                         onerror="this.onerror=null; this.src='${fallbackPath}'; this.onerror=function(){this.style.display='none'; this.parentElement.innerHTML='<span class=\\'fallback-char\\'>${fallbackChar}</span>';}">
                </div>
...
function showDetail(kanji) {
    ...
    // Use the level property from the data, fallback to 'n3'
    const levelClass = d.level || 'n3';
    const svgPaths = getSvgPathWithFallback(d.unicode);
    const fallbackChar = d.kanji;

    // Build readings HTML with clickable individual readings
    let readingsHtml = '';
    
    // On'yomi row with clickable items
    if (d.onyomi) {
        readingsHtml += buildReadingHTML(d.onyomi, true);
    }
    
    // Kun'yomi row with clickable items
    if (d.kunyomi) {
    ...
    let html = `
        <!-- Kanji + SVG side-by-side -->
        <div class="detail-header">
            <div class="detail-kanji">${d.kanji}</div>
            <div class="detail-stroke">
                <img src="${svgPaths.primary}" alt="Stroke order for ${d.kanji}" 
                     onerror="this.src='${svgPaths.fallback}'; this.onerror=function(){this.style.display='none'; this.parentElement.innerHTML='<span class=\\'fallback-char\\'>${fallbackChar}</span>';}">
            </div>
        </div>
    ...
    // TTS for individual reading items (click)
    if (ttsEnabled) {
        ...
        // TTS for sentence container - click on the whole example
    ...
    // Add to selection - clean button without pay icon
    const addBtn = dom.detailContent.querySelector('.btn-practice');
    if (addBtn) {
        addBtn.addEventListener('click', (e) => {
            e.stopPropagation(); // Prevent sentence TTS from triggering
...
function generatePrint() {
    ...
    data.forEach(d => {
        const svgPath5 = getSvgPathForPrint(d.unicode);
        const svgPath4 = `images/kanji-strokes/${d.unicode.replace(/^0+/, '')}.svg`;
        html += `
            <div class="print-entry">
                <div class="print-kanji">
                    ${d.kanji}
                    <div class="print-stroke">
                        <img src="${svgPath5}" alt="Stroke order for ${d.kanji}" 
                             onerror="this.src='${svgPath4}'; this.onerror=function(){this.style.display='none'; this.parentElement.innerHTML='<span style=\\'font-size:20px;color:#ccc;\\'>${d.kanji}</span>';}">
                    </div>
                </div>
